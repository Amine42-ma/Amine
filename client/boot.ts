...
import { connect, send, onMessage, loadTiles, useTransport, type Transport } from './net.js';
...
/**
 * The game front end: render loop, input handling, and the mapping from server
 * messages into the store. Movement is predicted locally and reconciled against
 * the authoritative position.
 *
 * It is deliberately agnostic about where the world lives. The networked build
 * points it at a WebSocket; the single-file build points it at a simulation
 * running in the same page. Nothing below needs to know which.
 */

export interface BootOptions {
  /** Supply a transport to run against an in-page world instead of a server. */
  transport?: Transport;
  /** Where to get the terrain grid from; defaults to fetching it from the server. */
  tiles?: () => Promise<Uint8Array> | Uint8Array;
  /** Skip the password field when there are no accounts to protect. */
  offline?: boolean;
}
...
function initGate(offline: boolean) {
  ...
  const submit = (mode: 'login' | 'register') => {
    ...
    const password = offline ? 'offline' : passInput.value;
    if (name.length < 2) { error.textContent = t('errShortName'); return; }
    if (!offline && password.length < 4) { error.textContent = t('errShortPassword'); return; }
...
export async function boot(options: BootOptions = {}) {
  initLang();
  applyStaticStrings();
  initGate(options.offline ?? false);
  initHud();
  initPanels();

  if (options.transport) useTransport(options.transport);
  else connect();

  try {
    store.tiles = await (options.tiles ? options.tiles() : loadTiles());
  } catch (err) {
