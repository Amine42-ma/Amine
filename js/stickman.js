...
class Stickman {
  ...
  update(opponent) {
    ...
    if (moveInput !== 0) {
      ...
      if (grounded) {
        ...
        if (Math.sign(vx) === -Math.sign(moveInput) && Math.abs(vx) > 0.4) {
    ...
    // Ground tracking with coyote time so jumping stays reliable even if the
    // fighter briefly loses foot contact (the cause of "it won't jump").
    if (grounded) { this._coyote = 8; if (this._jumpLock <= 0) this.jumpsLeft = this.maxJumps; }
    else if (this._coyote > 0) this._coyote--;

    // Jump — rising edge, with a double jump in the air.
    // Launch the WHOLE body (incl. feet & knees) upward together so the feet
    // actually leave the ground; impulsing only the pelvis let the grounded
    // feet + leg constraints hold it down (the "won't jump" bug).
  ...
  drawHead(ctx, color) {
    const h = this.head;
    // Orient the head so its "up" follows the spine (head above chest). Using
    // head→chest here previously added a 180° flip that made the face point at
    // the sky; head−chest keeps it upright and looking forward.
    const ang = Math.atan2(h.y - this.chest.y, h.x - this.chest.x) + Math.PI / 2;
    ctx.save();
